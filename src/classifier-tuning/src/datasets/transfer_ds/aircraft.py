...
class FGVCAircraft_split(data.Dataset):
    ...
    def __init__(self, root, class_type='variant', train=True, transform=None,
        ...
        new_ds = '/opt/tiger/filter_transfer/data/aircraft_train_val'
        os.system('mkdir -p {}/val/'.format(new_ds))
        print('mkdir -p {}/val/'.format(new_ds))

        idx_to_class = {v:k for k,v in class_to_idx.items()}

        for cls in classes:
            cls = cls.replace(' ', '_').replace('/', '_')
            os.system('mkdir {}/val/{}'.format(new_ds, cls[:-1]))
            print('mkdir {}/val/{}'.format(new_ds, cls[:-1]))

        for s in samples:
            im_path, cls_idx = s
            im_cls = idx_to_class[cls_idx][:-1]
            im_cls = im_cls.replace(' ', '_').replace('/', '_')
            os.system('cp {} {}/val/{}'.format(im_path, new_ds, im_cls))
            print('cp {} {}/val/{}'.format(im_path, new_ds, im_cls))
...
if __name__ == '__main__':
    # train_dataset = FGVCAircraft(root='/opt/tiger/filter_transfer/data/fgvc-aircraft-2013b', train=True)
    train_dataset = FGVCAircraft(root='/opt/tiger/filter_transfer/data/fgvc-aircraft-2013b', train=False)